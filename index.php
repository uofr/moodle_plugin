...
$strplural = get_string("modulenameplural", "mod_kalvidpres");            // Video presentation.

$PAGE->set_url('/mod/kalvidpres/index.php', array('id' => $course->id));
...
if (!$vidres = get_all_instances_in_course('kalvidpres', $course)) {
    notice(get_string('noresource', 'mod_kalvidpres'), "$CFG->wwwroot/course/view.php?id=$course->id");
    exit;
}
...
foreach ($vidres as $res) {
    ...
    $icon = $OUTPUT->pix_icon('icon', get_string('modulename', 'mod_kalvidpres'), 'mod_kalvidpres');
    ...
    $table->data[] = array (
        ...
        format_module_intro('kalvidpres', $res, $res->coursemodule)
    );
